...
| :--- | :--- | :---: | :---: | :--- | :--- |
| **M4-00** | **Contract Design** | Current Agent | **`DONE`** | M3 Acceptance | `docs/M4_*.md` (Design & Contract Freeze) |
...
| **M4-04** | **Entity & Classification Attachment** | TBD | **`TODO`** | M4-03 | `src/knowledge/enrichment.py`, `tests/test_knowledge_enrichment.py` |
| **M4-05** | **Knowledge Document & Markdown Render** | TBD | **`TODO`** | M4-04 | `src/knowledge/render.py`, `tests/test_knowledge_render.py` |
...
### M4-00: Contract Design (`DONE`)
- **Objective**: Freeze Canonical KnowledgeUnit contract, schema, taxonomy, epistemic semantics, and task plan without modifying production code.
- **Deliverables**:
  - `docs/M4_KNOWLEDGE_MODEL_DESIGN.md`: Comprehensive 15-section design specification.
...
### M4-04: Entity & Classification Attachment (`TODO`)
...
### M4-05: Knowledge Document & Markdown Render (`TODO`)
- **Objective**: Serialize canonical knowledge artifacts to disk.
- **Target Scope**:
  - Generate `data/processed/<canonical_id>/knowledge/knowledge_units.json` (schema: `knowledge-units-v1`).
  - Generate human-readable `data/processed/<canonical_id>/knowledge/knowledge.md` linking claims to exact evidence excerpts.
